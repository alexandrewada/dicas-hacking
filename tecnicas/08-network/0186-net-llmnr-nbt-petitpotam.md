...
aliases: ["coerção + relay", "petitpotam"]
---
...
# trigger petitpotam; evidência: auth USER_A + ação não destrutiva tag 8528a4
```
...
coerção + relay: se não reproduz efeito (authz/dado/exec), não infla severidade. Referência de sinal: Detectar Rogue WPAD/LLMNR responders; SMB signing compliance.
