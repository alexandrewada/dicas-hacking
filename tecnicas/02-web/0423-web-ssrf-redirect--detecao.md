...
aliases: ["bypass via redirect aberto", "redirect", "redirect-detecao"]
---

# bypass via redirect aberto — detecção

Purple em bypass via redirect aberto: uma execução limpa. A pergunta é se alertou — não se o exploit 'passa'.
...
log: src=app dst=internal-admin.lab.local action=ALLOW? → gap (redirect/e71c06)
```
