...
aliases: ["filename path traversal", "traversal-name", "traversal-name-path"]
---

# filename path traversal — path

filename path traversal como pivô. Path curto > monte de finding isolado.
...
1. Entrada (escopo)
2. Pivô: filename path traversal
3. Objetivo do ROE
...
Host: app.lab.local
Content-Type: multipart/form-data; boundary=----3dc046
...
- [filename path traversal — detecção](0434-web-upload-traversal-name--detecao.md)
