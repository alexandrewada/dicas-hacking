...
aliases: ["sessão pré-MFA reutilizada", "session-upgrade", "session-upgrade-detecao"]
---

# sessão pré-MFA reutilizada — detecção

Gap de detecção em `T1621 Multi-Factor Authentication Request Generation` / sessão pré-MFA reutilizada. PoC mínimo, telemetria ligada.
...
app=APP_LAB flow=session-upgrade tag=333d30
```
