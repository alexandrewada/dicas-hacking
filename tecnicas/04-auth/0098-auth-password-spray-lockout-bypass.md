...
aliases: ["bypass de lockout por pool de IPs", "lockout-bypass"]
---

# bypass de lockout por pool de IPs
...
```http
GET /oauth/authorize?client_id=APP_LAB&redirect_uri=https://evil.lab.local/cb&response_type=code&state=20bb06 HTTP/1.1
Host: idp.lab.local
...
Já abri High demais em bypass de lockout por pool de IPs por sintoma sem efeito. Cruzei com: Alertas de auth failures distribuídos; impossible spray patterns; MFA coverage. Sem side-effect, baixo.
...
- [bypass de lockout por pool de IPs — path](0858-auth-password-spray-lockout-bypass--path.md)
