...
aliases: ["CSRF de login sem state", "state", "state-path"]
---

# CSRF de login sem state — path

CSRF de login sem state como pivô. Path curto > monte de finding isolado.
...
1. Entrada (escopo)
2. Pivô: CSRF de login sem state
3. Objetivo do ROE
...
```http
GET /oauth/authorize?client_id=APP_LAB&redirect_uri=https://evil.lab.local/cb&response_type=code&state=9b1834 HTTP/1.1
Host: idp.lab.local
