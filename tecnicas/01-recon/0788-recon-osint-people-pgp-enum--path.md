...
aliases: ["enumeração via diretórios PGP", "pgp-enum", "pgp-enum-path"]
---

# enumeração via diretórios PGP — path

enumeração via diretórios PGP como pivô. Path curto > monte de finding isolado.
...
1. Entrada (escopo)
2. Pivô: enumeração via diretórios PGP
3. Objetivo do ROE
...
# marcar dev-/staging- ; tag ee7cfa (pgp-enum)
```
...
- [enumeração via diretórios PGP — detecção](0408-recon-osint-people-pgp-enum--detecao.md)
