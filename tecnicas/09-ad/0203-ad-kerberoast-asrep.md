...
**Variante:** **Pre-auth disabled** — muda ruído e o que entra no PDF. DONT_REQ_PREAUTH = AS-REP roast sem SPN. Confirmo UAC no LDAP e limito amostra. Priorizo serviço com SPN + password fraca. Dump de todo o domínio é amador e barulhento.
...
GetUserSPNs.py lab.local/USER_A:PASS_LAB -dc-ip DC01.lab.local -request \
  -outputfile roast_asrep_ec9187.kirbi
...
AS-REP roasting: se não reproduz efeito (authz/dado/exec), não infla severidade. Referência de sinal: Event 4769 anômalos; honeypot SPNs; AES-only policies.
